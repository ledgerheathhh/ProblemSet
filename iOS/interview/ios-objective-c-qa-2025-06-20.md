...
官方解析：
block的本质是一个OC对象，编译成C++代码后，是个结构体，里面有isa指针，封装了函数调用环境和函数。block有3种类型：
- **全局block（_NSGlobalBlock__）**：不捕获变量，或只捕获全局/静态变量，存储在数据区
- **栈block（_NSStackBlock__）**：捕获局部变量但未copy，存在于栈上
- **堆block（_NSMallocBlock__）**：对栈block执行copy后复制到堆上

因此，准确地说：**捕获局部变量的block经copy操作后才会存在堆上**（ARC环境下可能自动copy）。原解析"block捕获了局部变量是堆block"表述不够严谨。
...
A、只有一个单例方法：`+[NSNull null]`

B、可以用于表示集合中值为空的对象

C、向NSNull对象发送消息时，返回NO

D、向NSNull的对象发送消息时会收到异常

正确答案：ABD

NSNull是iOS开发中一个特殊的类,用于在集合对象中表示空值。

选项A正确:NSNull确实只提供了一个类方法+[NSNull null]来获取其单例对象,这是使用NSNull的唯一方式。

选项B正确:NSNull的主要用途就是在NSArray、NSDictionary等集合中代表nil值,因为这些集合不能直接存储nil。当我们需要在集合中表示"空"时,就可以使用[NSNull null]。

选项C错误:向NSNull对象发送不支持的消息时会触发"未识别的选择器"异常,而不是简单返回NO。NSNull并没有默认的消息处理机制。

选项D正确:因为NSNull类只实现了非常有限的方法,当我们向NSNull对象发送它未实现的消息时,会抛出NSInvalidArgumentException异常。

总结来说,NSNull是一个用于在集合中表示空值的特殊类,它通过单例模式实现,具有严格的消息处理机制。当收到未实现的消息时会抛出异常,而不是返回默认值。这些特性使得NSNull在需要在集合中表示空值的场景下非常有用。